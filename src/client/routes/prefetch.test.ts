...
import { observeRoutePrefetch, prefetchAfterHandler, prefetchRoute, prefetchRootMargin } from "./prefetch";
...
describe("route prefetch behavior", () => {
    ...
    it("prefetches on keyboard-focus intent", async () => {
        let preloads = 0;
        prefetchAfterHandler({ defaultPrevented: false, type: "focus" }, undefined, async () => {
            preloads += 1;
        });
    ...
    it("prefetches on pointer-down intent", async () => {
        let preloads = 0;
        prefetchAfterHandler({ defaultPrevented: false, type: "pointerdown" }, undefined, async () => {
            preloads += 1;
        });
    ...
    it("observes with the selected root margin and stops after success", async () => {
        ...
        const cleanup = observeRoutePrefetch({} as Element, async () => undefined, undefined, ObserverStub);
