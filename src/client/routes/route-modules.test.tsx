...
describe("lazy route registry", () => {
    it("points important paths at their shared route module preloaders", () => {
        ...
        expect(routePreloaders[routes.osuguessr]).toBe(routeModules.osuguessr.preload);
        expect(routePreloaders[routes.profilePrivacy]).toBe(routeModules.accountPrivacy.preload);
        expect(routePreloaders[routes.legalPrivacy]).toBe(routeModules.legalPrivacy.preload);
        expect(routePreloaders[routes.legalTerms]).toBe(routeModules.legalTerms.preload);
    });

    it("loads every unique lazy page module without module-level side effects", async () => {
        const loaded = await Promise.all(Object.values(routeModules).map((module) => module.preload()));
        expect(loaded.every((module) => typeof module.default === "function")).toBe(true);
    });
