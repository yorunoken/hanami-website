...
const commands = [
  [
    "/profile",
    "Player profiles",
    "Rank, accuracy, play count, country, and account details.",
  ],
  [
    "/recent",
    "Recent plays",
    "The latest score with mods, combo, hit results, and performance data.",
  ],
  ["/top", "Top scores", "Best plays for a linked user or an osu! username."],
  [
    "/beatmap",
    "Beatmap details",
    "Difficulty, BPM, length, object counts, estimates, and map links.",
  ],
  [
    "/compare",
    "Score comparison",
    "Compare a player on a beatmap already shared in the conversation.",
  ],
  [
    "/whatif",
    "Performance scenarios",
    "Explore performance-point requirements for different score outcomes.",
  ],
] as const;
...
export default function Bot() {
  ...
  return (
    <ProductPage>
      <ProductHero className="bg-[linear-gradient(125deg,#121015_0%,#171019_68%,#22111a_100%)]">
        <div className={productHeroCopyClass}>
          ...
          <StatusLine
            ...
            detail="Slash and prefix commands"
            tone={product.tone}
          />
          <h1 className={productTitleClass}>{product.name}</h1>
          <h2 className={productSubtitleClass}>
            osu! information where the conversation is already happening.
          </h2>
          <p className={productBodyClass}>
            Look up players, scores, and beatmaps inside Discord. Link an osu!
            account once to use your own profile as the default in supported
            commands.
          </p>
          <HeroActions>
      ...
      <ProductSection aria-labelledby="bot-commands-title">
        ...
        <div className="border-t border-border-strong" id="bot-commands-title">
          {commands.map(([command, title, description], index) => (
            ...
            >
              <span
                className="font-mono text-[0.7rem] text-quiet"
                aria-hidden="true"
              >
                {String(index + 1).padStart(2, "0")}
              </span>
              <code className="font-mono text-[0.86rem] text-accent-soft">
                {command}
              </code>
              <div className="max-[600px]:col-start-2">
                <h3 className="text-base tracking-[-0.02em]">{title}</h3>
                <p className="mt-[0.35rem] max-w-[62ch] text-[0.9rem] leading-[1.65] text-muted">
                  {description}
                </p>
              </div>
            </article>
      ...
      <ProductSplit>
        <div>
          ...
          <h2 className={sectionHeadingClass}>
            Use your own profile without typing it every time.
          </h2>
          <p className={sectionBodyClass}>
            Sign in to Hanami with Discord, authorize osu! identity access, then
            choose bot display preferences from the account page.
          </p>
        ...
        <ProductSteps>
          <ProductStep icon={<Link2 aria-hidden="true" />}>
            ...
            <p>
              Hanami stores the Discord-to-osu! ID link in the bot database.
            </p>
          </ProductStep>
          ...
          <ProductStep icon={<BotIcon aria-hidden="true" />}>
            ...
            <p>
              The bot resolves supported commands against the linked account.
            </p>
          </ProductStep>
        </ProductSteps>
      </ProductSplit>
      ...
      <ProductFootnote>
        <p>
          <strong>Privacy note:</strong> unlinking osu! removes the ID link; it
          does not delete the web account, bot settings, or other service data.
        </p>
        <TextLink href="/legal/data-deletion">
