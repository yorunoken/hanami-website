...
const outputs = [
  [
    TerminalSquare,
    "Readable reports",
    "Compact terminal output by default, with grouped detail on request.",
  ],
  [
    Braces,
    "Rust types",
    "Structured timing, object, rhythm, stream, aim, speed, slider, and section results.",
  ],
  [
    FileJson2,
    "Dataset exports",
    "JSON, JSONL, and CSV paths for analysis and labeling workflows.",
  ],
  [
    CheckCircle2,
    "Validation",
    "Structural and numerical checks with separate passes, warnings, and errors.",
  ],
] as const;
      ...
      <ProductHero className="bg-[linear-gradient(rgba(180,218,122,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(180,218,122,0.045)_1px,transparent_1px),#0a0d0a] bg-[length:48px_48px]">
        <div className={productHeroCopyClass}>
          ...
          <StatusLine
            ...
            detail="Crate version 0.2.9"
            tone={product.tone}
          />
          <h1 className={productTitleClass}>{product.name}</h1>
          <h2 className={productSubtitleClass}>
            Turn a local .osu file into inspectable structure.
          </h2>
          <p className={productBodyClass}>
            A Rust library and command-line tool for local beatmap analysis,
            human-readable reports, validation, and dataset export. The analyzer
      ...
      <ProductSplit>
        <div>
          ...
          <p className={sectionBodyClass}>
            The published package is Apache-2.0 licensed and built on rosu-map
            and rosu-pp.
          </p>
          <ActionLink
            className="mt-[1.8rem]"
            href={product.links.crate ?? product.links.primary}
            external
          >
