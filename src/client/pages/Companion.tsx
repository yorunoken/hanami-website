...
const currentCapabilities = [
  ...
  [
    "Local state events",
    "It listens for local play-state changes over tosu’s WebSocket.",
  ],
] as const;
  ...
  return (
    <ProductPage>
      <ProductHero className="bg-[linear-gradient(130deg,rgba(128,215,232,0.08),transparent_42%),#0b0c0f]">
        <div className={productHeroCopyClass}>
          ...
          <StatusLine
            ...
            detail="Source only; no packaged release"
            tone={product.tone}
          />
          ...
          <p className={productBodyClass}>
            The current Tauri prototype can manage tosu and observe local osu!
            state. Hanami account connection and score upload are still mocked
            in code.
          </p>
        ...
        <figure className="text-center motion-safe:animate-[reveal-up_550ms_150ms_cubic-bezier(0.2,0.7,0.2,1)_both]">
          ...
          <figcaption className="mx-auto mt-4 max-w-[45ch] text-[0.72rem] leading-[1.55] text-quiet max-[600px]:text-left">
            Current project icon. The desktop application is not yet
            distributed.
          </figcaption>
        </figure>
      </ProductHero>
        ...
        <div className="grid grid-cols-1 border-t border-border-strong min-[821px]:grid-cols-3">
          {currentCapabilities.map(([title, description], index) => (
            ...
            >
              ...
              <h3 className="mt-auto text-base tracking-[-0.02em] text-[#c4f1f7]">
                {title}
              </h3>
      ...
      <ProductSplit>
        <div>
          ...
          <h2 className={sectionHeadingClass}>
            Local first, with the network path still unfinished.
          </h2>
          <p className={sectionBodyClass}>
            The prototype connects to a local tosu WebSocket. Its Hanami API
            client only prints mock score and map-upload messages; the network
      ...
      <ProductFootnote className="[&>svg]:text-cyan">
        ...
        <p>
          No release download is offered because the repository is still a
