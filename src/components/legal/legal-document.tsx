...
export function LegalDocument({
  ...
  return (
    <main
      className={cn(
        siteContainerClass,
        "py-[clamp(4rem,8vw,7rem)] print:w-full print:py-0",
      )}
    >
      <header className="max-w-[900px] border-b border-border-strong pb-[clamp(2.5rem,5vw,4rem)] print:max-w-none">
        ...
        <h1 className="text-[clamp(2.7rem,6vw,5.1rem)] leading-none tracking-[-0.06em] print:text-[#111]">
          {title}
        </h1>
        <p className="mt-[1.35rem] max-w-[68ch] text-base leading-[1.7] text-muted print:text-[#333]">
          {summary}
        </p>
        <dl className="mt-8 flex flex-wrap gap-x-12 gap-y-6">
          <div>
            <dt className="mb-[0.3rem] font-mono text-[0.64rem] tracking-[0.06em] text-quiet uppercase">
              Effective date
            </dt>
            <dd className="text-[0.8rem] text-[#d6d0d7] print:text-[#333]">
        ...
        <p className="mt-[1.8rem] max-w-[76ch] border-l-2 border-accent pl-4 text-[0.82rem] leading-[1.65] text-[#c9c1ca] [&_code]:font-mono [&_code]:text-[0.88em] [&_code]:font-semibold [&_code]:text-[#ffd0e3]">
          <strong>Draft notice:</strong> unresolved facts are marked{" "}
          <code>REQUIRES OWNER CONFIRMATION</code>. This text must be reviewed
          before publication as an effective policy.
        </p>
      </header>
      ...
      <div className="mt-16 grid grid-cols-1 items-start gap-10 min-[821px]:grid-cols-[210px_minmax(0,760px)] min-[821px]:gap-[clamp(3rem,8vw,7rem)] print:mt-8 print:block">
        ...
        >
          <h2 className="mb-4 font-mono text-[0.68rem] tracking-[0.08em] text-quiet uppercase">
            On this page
          </h2>
          <ol className="columns-1 min-[601px]:columns-2 min-[821px]:columns-1">
            {toc.map((item) => (
              <li
                className="break-inside-avoid [&+li]:mt-[0.65rem]"
                key={item.id}
              >
                <a
                  className="text-[0.75rem] leading-[1.4] text-muted no-underline hover:text-white"
                  href={`#${item.id}`}
                >
                  {item.label}
                </a>
...
export function LegalTable({ children }: { children: ReactNode }) {
  return (
    ...
    >
      <table className="w-full min-w-[620px] border-collapse text-[0.78rem] [&_td]:border-b [&_td]:border-border [&_td]:px-3 [&_td]:py-[0.85rem] [&_td]:text-left [&_td]:align-top [&_td]:leading-[1.55] [&_td]:text-[#bbb4bd] [&_th]:border-b [&_th]:border-border [&_th]:px-3 [&_th]:py-[0.85rem] [&_th]:text-left [&_th]:align-top [&_th]:leading-[1.55] [&_th]:font-bold [&_th]:text-white print:[&_td]:text-[#333] print:[&_th]:text-[#111]">
        {children}
      </table>
    </div>
  );
