        ...
        <article className={identityBlockClass}>
          {loading ? (
            ...
            <>
              <div className={identityPersonClass}>
                <Avatar
                  src={linkStatus.avatarUrl}
                  name={linkStatus.username || "osu! player"}
                />
                <div>
                  <p>osu! link</p>
                  <h3>
                    <a
                      href={`https://osu.ppy.sh/users/${linkStatus.banchoId}`}
                      target="_blank"
                      rel="noreferrer"
                    >
            ...
            <>
              ...
              <p className="my-8 text-[0.88rem] leading-[1.65] text-muted">
                Connect an osu! account to let supported bot commands use your
                profile by default.
              </p>
              <button
  ...
  return (
    <section className="mt-16" aria-labelledby="settings-title">
      <div className={sectionHeadingClass}>
        ...
        <p>
          These values are stored against your Discord ID in the bot database.
        </p>
      </div>
        ...
        <form className="pt-8" onSubmit={onSubmit}>
          <div className="grid grid-cols-1 gap-x-12 gap-y-6 min-[601px]:grid-cols-2 min-[601px]:gap-y-8">
            <SelectField
              ...
              value={current.mode}
              onChange={(value) =>
                onSettingsChange({ ...current, mode: parseMode(value) })
              }
            >
          ...
          <div className="mt-10 flex items-center gap-6 border-t border-border pt-6 max-[600px]:flex-col max-[600px]:items-start">
            ...
            {saved && (
              <span
                className={cn(formMessageClass, "text-success")}
                role="status"
              >
...
export function AccountPrivacyAside() {
  return (
    <aside className="mt-16 max-w-[760px] border-l-2 border-accent pl-6 [&_a]:text-[0.82rem] [&_a]:text-white [&_a]:underline-offset-[0.2em] [&_h2]:text-[1.1rem] [&_p]:my-[0.7rem] [&_p]:text-[0.84rem] [&_p]:leading-[1.65] [&_p]:text-muted">
      ...
      <p>
        Review what Hanami may hold, submit a verified deletion request, or
        check a request already in progress.
      </p>
