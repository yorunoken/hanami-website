...
export const accountPageClass = cn(
  sitePageClass,
  "bg-[linear-gradient(150deg,rgba(235,118,170,0.055),transparent_34%),var(--color-bg)]",
);

export const accountHeadingClass =
  "mb-16 max-w-[780px] [&>h1]:text-[clamp(2.6rem,6vw,5rem)] [&>h1]:leading-none [&>h1]:tracking-[-0.06em] [&>p:not(:first-child)]:mt-5 [&>p:not(:first-child)]:max-w-[62ch] [&>p:not(:first-child)]:leading-[1.7] [&>p:not(:first-child)]:text-muted";
...
export function AccountLayout({
  ...
  return (
    <main
      className={cn(siteContainerClass, "py-[clamp(4rem,8vw,7rem)]", className)}
    >
      {children}
    </main>
  );
  ...
  return (
    <AccountPage>
      <main
        className={cn(
          siteContainerClass,
          "grid min-h-[620px] place-items-center py-20",
        )}
      >
        {children}
      </main>
