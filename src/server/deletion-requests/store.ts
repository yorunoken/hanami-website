...
export type StoreFailureCode = "challenge_invalid" | "challenge_stale" | "duplicate_active" | "not_cancellable" | "not_found";
...
export interface DeletionRequestStore {
    ...
    createRequest(input: { userId: string; tokenHash: string; now: Date }): Promise<PublicDeletionRequest>;
...
export class MySqlDeletionRequestStore implements DeletionRequestStore {
    ...
    async getAccountSummary(userId: string): Promise<AccountDeletionSummary> {
        const [accountRows, requestRows] = await Promise.all([
            this.pool.execute<AccountRow[]>("SELECT accountId FROM account WHERE userId = ? AND providerId = 'discord' LIMIT 1", [userId]),
        ...
        return {
            ...
            request: requestRows[0][0] ? toPublicDeletionRequest(mapRequest(requestRows[0][0])) : null,
        };
    }
    ...
    async completeReauthentication(input: { userId: string; tokenHash: string; sessionCreatedAt: Date; now: Date }): Promise<Date> {
        return withTransaction(this.pool, async (connection) => {
          ...
          WHERE id = ? AND reauthenticatedAt IS NULL AND consumedAt IS NULL`,
            ...
            if (result.affectedRows !== 1) throw new DeletionRequestStoreError("challenge_invalid");
            return input.now;
        });
    }
    ...
    private async createRequestOnce(
        ...
        return withTransaction(this.pool, async (connection) => {
          ...
          FOR UPDATE`,
            ...
            if (activeRows[0]) throw new DeletionRequestStoreError("duplicate_active");
         ...
         VALUES (?, ?, 'pending', ?, ?, ?, NULL, NULL, NULL, NULL, ?)`,
                [id, input.userId, input.now, challenge.reauthenticatedAt, input.now, requestReference],
            );
          ...
          WHERE id = ? AND consumedAt IS NULL`,
            ...
            if (challengeResult.affectedRows !== 1) throw new DeletionRequestStoreError("challenge_invalid");
            ...
            await connection.execute("DELETE FROM session WHERE userId = ?", [input.userId]);
    ...
    async cancelRequest(userId: string, now: Date): Promise<PublicDeletionRequest> {
        return withTransaction(this.pool, async (connection) => {
          ...
          FOR UPDATE`,
            ...
            if (request.status !== "pending" && request.status !== "in_review") throw new DeletionRequestStoreError("not_cancellable");
          ...
          WHERE id = ? AND status IN ('pending', 'in_review')`,
            ...
            if (result.affectedRows !== 1) throw new DeletionRequestStoreError("not_cancellable");
...
function assertChallengeUsable(challenge: ChallengeRow, now: Date): void {
    if (challenge.consumedAt) throw new DeletionRequestStoreError("challenge_invalid");
    if (challenge.expiresAt.getTime() <= now.getTime()) throw new DeletionRequestStoreError("challenge_stale");
}
...
function isReferenceCollision(error: unknown): boolean {
    return isMysqlError(error) && error.errno === 1062 && error.message.includes("accountDeletionRequest_reference_unique");
}

function isMysqlError(error: unknown): error is { errno: number; message: string } {
    return (
        typeof error === "object" &&
        error !== null &&
        "errno" in error &&
