...
export const deletionRequestStatuses = ["pending", "in_review", "processing", "completed", "rejected", "cancelled", "failed"] as const;
...
export function createRequestReference(): string {
    const random = Buffer.from(crypto.getRandomValues(new Uint8Array(15))).toString("base64url");
    return `HAN-${random}`;
}

export function toPublicDeletionRequest(record: DeletionRequestRecord): PublicDeletionRequest {
    return {
