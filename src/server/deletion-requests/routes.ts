...
import {
  DeletionRequestStoreError,
  MySqlDeletionRequestStore,
...
export function createDeletionRequestRoutes(
  dependencies: DeletionRequestRouteDependencies = productionDependencies,
) {
  return new Elysia({ prefix: "/deletion-requests" })
    ...
    .post("/reauth/start", async ({ request, set }) => {
      ...
      if (!hasValidOrigin(request))
        return fail(set, 403, "This request could not be verified.");

      try {
        const summary = await dependencies.store.getAccountSummary(
          session.user.id,
        );
        if (
          summary.request &&
        ...
        const alreadyFresh = isFreshAuthentication(
    ...
    .post("/reauth/complete", async ({ request, body, set }) => {
      ...
      if (!challenge)
        return fail(set, 400, "The reauthentication request is invalid.");

      try {
        const now = dependencies.now();
        const reauthenticatedAt =
          await dependencies.store.completeReauthentication({
            userId: session.user.id,
            tokenHash: await hashChallengeToken(challenge),
    ...
    .post("/", async ({ request, body, set }) => {
      ...
      try {
        ...
        return handleStoreFailure(
          set,
          error,
          "The account deletion request could not be submitted.",
        );
      }
    })
    .post("/cancel", async ({ request, set }) => {
      ...
      try {
        return {
          request: await dependencies.store.cancelRequest(
            session.user.id,
            dependencies.now(),
          ),
        };
        ...
        return handleStoreFailure(
          set,
          error,
          "The account deletion request could not be cancelled.",
        );
      }
    });
}
    ...
    switch (error.code) {
      ...
      case "challenge_stale":
        return fail(
          set,
          403,
          "Fresh Discord authentication is required. Please start again.",
        );
...
function logFailure(action: string, error: unknown): void {
  console.error(
    ...
    error instanceof Error
      ? { name: error.name, code: readErrorCode(error) }
      : { type: typeof error },
  );
}
