...
describe("account deletion request routes", () => {
  ...
  it("requires Discord reauthentication for a stale session", async () => {
    ...
    expect(data.confirmationPath).toStartWith(
      "/profile/privacy/confirm#challenge=",
    );
  });
  ...
  it("lets a freshly authenticated user reach confirmation", async () => {
    ...
    expect(data.confirmationPath).not.toContain(
      store.lastStart?.tokenHash ?? "",
    );
  });
  ...
  it("creates a request for a valid confirmed user", async () => {
    ...
    expect(store.lastCreate?.tokenHash).toBe(
      await hashChallengeToken(challenge),
    );
  });
  ...
  it("scopes status reads to the authenticated user", async () => {
    ...
    const response = await app.handle(
      new Request("http://localhost/deletion-requests"),
    );
  ...
  it("does not reflect raw internal errors", async () => {
    ...
    store.createError = new Error(
      "ER_ACCESS_DENIED production-host password=secret stack",
    );
    const { app } = makeApp({ store });
    const response = await post(app, "/deletion-requests", {
      challenge: "a".repeat(43),
...
function post(
  app: ReturnType<typeof createDeletionRequestRoutes>,
  path: string,
