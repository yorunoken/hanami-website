...
const pool = testDatabaseUrl
  ? mysql.createPool({ uri: testDatabaseUrl, timezone: "Z" })
  : null;
...
describeDatabase("MySQL account deletion request store", () => {
  ...
  it("creates a durable request, consumes reauthentication, and revokes sessions", async () => {
    ...
    const [challengeRows] = await pool.execute<
      Array<RowDataPacket & { consumedAt: Date | null }>
    >(
  ...
  it("cancels only before processing", async () => {
    ...
    await pool.execute(
      "UPDATE accountDeletionRequest SET status = 'processing', cancelledAt = NULL WHERE userId = ?",
      ["user-1"],
    );
    await expect(
      store.cancelRequest("user-1", new Date(now.getTime() + 3_000)),
