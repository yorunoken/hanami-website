...
describe("Legacy legal URLs", () => {
  it("redirects old privacy URLs to the legal center", async () => {
    const response = await app.handle(
      new Request("http://localhost/privacy?source=footer"),
    );
    expect(response.status).toBe(308);
    expect(response.headers.get("location")).toBe(
      "http://localhost/legal/privacy?source=footer",
    );
  });

  it("redirects old terms URLs to the legal center", async () => {
    const response = await app.handle(
      new Request("http://localhost/terms-of-service"),
    );
    expect(response.status).toBe(308);
    expect(response.headers.get("location")).toBe(
...
function restoreEnvironment(
  key: "OSU_CLIENT_ID" | "OSU_CALLBACK_URL" | "BETTER_AUTH_SECRET",
