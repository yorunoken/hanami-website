  ...
  .get("/osu-link/status", async ({ request, set }) => {
    ...
    try {
      return await withDatabases(async (webDb, botDb) => {
        ...
        const [users] = await botDb.execute<BotLinkRow[]>(
          "SELECT banchoId FROM users WHERE id = ?",
          [discordId],
        );
  ...
  .delete("/osu-link/unlink", async ({ request, set }) => {
    ...
    try {
      return await withDatabases(async (webDb, botDb) => {
        ...
        await botDb.execute("UPDATE users SET banchoId = NULL WHERE id = ?", [
          discordId,
        ]);
        return { success: true };
      });
...
async function withDatabases<T>(
  callback: (webDb: Connection, botDb: Connection) => Promise<T>,
): Promise<T> {
  let webDb: Connection | null = null;
  let botDb: Connection | null = null;
  try {
    webDb = await mysql.createConnection(
      process.env.WEB_DATABASE_URL as string,
    );
    botDb = await mysql.createConnection(
      process.env.BOT_DATABASE_URL as string,
    );
...
async function fetchPublicOsuProfile(banchoId: string): Promise<{
  ...
  try {
    ...
    const userResponse = await fetch(
      `https://osu.ppy.sh/api/v2/users/${encodeURIComponent(banchoId)}/osu`,
      {
        headers: {
    ...
    const statistics = isRecord(userData.statistics)
      ? userData.statistics
      : null;
    ...
    return {
      ...
      avatarUrl:
        typeof userData.avatar_url === "string"
          ? userData.avatar_url
          : `https://a.ppy.sh/${banchoId}`,
      globalRank:
        statistics && typeof statistics.global_rank === "number"
...
function parseSettings(value: unknown): BotSettings | null {
  ...
  if (!isOneOf(value.mode, ["osu", "mania", "taiko", "fruits"] as const))
    return null;
  if (!isOneOf(value.embed_type, ["hanami", "bathbot", "owobot"] as const))
    return null;
