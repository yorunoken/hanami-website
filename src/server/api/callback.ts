...
export const callbackRoute = new Elysia().get(
  "/callback",
  async ({ request, query, set, redirect }) => {
    ...
    if (!(await validateOAuthState(state, session.user.id, stateSecret))) {
      ...
      return {
        success: false,
        message:
          "The osu! authorization request expired or could not be verified. Please start again.",
      };
    }
    ...
    try {
      ...
      if (!tokenResponse.ok) {
        ...
        return {
          success: false,
          message:
            "osu! rejected or expired the authorization code. Please start again.",
        };
      }
      ...
      if (!isOsuTokenResponse(tokenData)) {
        console.error(
          "osu! token exchange returned an unexpected response shape",
        );
        set.status = 502;
        return {
          success: false,
      ...
      if (!isOsuIdentity(userData)) {
        console.error(
          "osu! identity request returned an unexpected response shape",
        );
        set.status = 502;
        return {
          success: false,
